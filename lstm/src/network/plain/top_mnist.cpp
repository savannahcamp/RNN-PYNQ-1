...
#pragma HLS STREAM variable=output_stream_dma_input depth=2
	...
	hls::stream<ap_uint<448> > temp("temp");
#pragma HLS STREAM variable=temp depth=2
...
#pragma HLS STREAM variable=output_stream_final_labeling depth=2
	...
	StreamingDataWidthConverter_Batch<DATAWIDTH, 448, 7>(output_stream_dma_input, temp, 112); // for W4A4 only

	StreamingDataWidthConverter_Batch<448, 112, 1>(temp, output_stream_columns, 112); // for W4A4 only
